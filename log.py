...
class Checkpoint():
    def __init__(self, args):
        ...
        if args['train']['save_results']:
            self.img_dir = os.path.join(self.log_dir, 'results')
            os.makedirs(self.img_dir, exist_ok=True)

        # 在 self.log_dir 下创建 test_img 目录
        self.test_img_dir = os.path.join(self.log_dir, 'test_img')
        ...
        # Saving yaml configs if it doesn't exist.
        if not os.path.exists(os.path.join(self.log_dir, 'configs.yaml')):
            with open(os.path.join(self.log_dir, 'configs.yaml'), 'w') as f:
                logger.info('[*] Saving run\'s yaml configs. Load/reset/resume/run is not saved if specified')
    ...
    def load_checkpoint(self, models, optimizers=None, schedulers=None):
        ...
        for load_dir in self.load_dirs:
            ...
            if os.path.exists(torch_path):
                ...
                # ✅ 始终使用CPU加载，避免显存占满和device不一致问题
                checkpoint = torch.load(torch_path, map_location='cpu')

                for model_typ in models.keys():
                    if model_typ in checkpoint['models']:
                        # ✅ 自动处理DataParallel保存时的module前缀问题
                        clean_dict = clean_state_dict(checkpoint['models'][model_typ])
                        self._load_model(models[model_typ], clean_dict)
